...
# %%
b = interactive(makeGICFailExample, Rfree=Rfree_widget, PermGroFac=PermGroFac_widget)
b
...
plt.tick_params(
    labelbottom=False, labelleft=False, left="off", right="off", bottom="off", top="off"
)
plt.xlim(0, 25)
...
if latexExists:
    plt.text(
        6,
        5,
        r"$\overline{\overline{c}}(m)= \overline{\MPC}m = (1-\wp^{1/\CRRA}\pmb{\text{\TH}}_{R})m$",
        fontsize=22,
        fontweight="bold",
    )
else:
    plt.text(
        6,
        5,
        r"$\overline{\overline{c}}(m)= \overline{\MPC}m = (1-\wp^{1/\CRRA}\Phi_{R})m$",
        fontsize=22,
        fontweight="bold",
    )
...
if latexExists:
    plt.text(
        5,
        0.87,
        r"$(1-\wp^{1/\CRRA}\pmb{\text{\TH}}_{R})\equiv \overline{\MPC}$",
        fontsize=26,
        fontweight="bold",
    )  # Use Thorn character
else:
    plt.text(
        5,
        0.87,
        r"$(1-\wp^{1/\CRRA}\Phi_{R})\equiv \overline{\MPC}$",
        fontsize=26,
        fontweight="bold",
    )  # Use Phi instead of Thorn (alas)
